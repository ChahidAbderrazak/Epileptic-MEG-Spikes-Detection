...
%% Script Starts
    cntm=1;Acc_op=0;cnt_inc=0;Qm=0;
...
%         [Accuracy,sz_fPWM]=Classify_LeaveOut_PWM(X,y,type_clf);starplus='StarPlus';
        [sz_fPWM, Accuracy,Avg_sensitivity,Avg_specificity,Avg_precision,Avg_gmean,Avg_f1score,Avg_AUC, QuPWM_str]=PWM_Data_CrossValidation(X, y,CV_type, K,type_clf);
        exec_time=toc;
    
        % Online diagnosis
        %% test on all data
        if  Qm~=M
            Q= mapping_levels(X0,Level_intervals, Levels);
            Qm=M;
        end
        
%         [accuracy,sensitivity,specificity,precision,gmean,f1score]=Clinical_Diagnosis_Sceneario(QuPWM_str, Q, y0);
...
%% Save the PWM classification results
    ...
    save(strcat(Path_classification,'Classification_results/',feature_type,pwm_param,noisy_file,suff,'_norm',num2str(Normalization),'_',CV_type,'_',type_clf,'_Acc',num2str(Acc_op),'.mat'),'pwm_param','feature_TAG','perform_output', 'noisy_file','*_all','list_*','*pd','mu0','sigma0',...
