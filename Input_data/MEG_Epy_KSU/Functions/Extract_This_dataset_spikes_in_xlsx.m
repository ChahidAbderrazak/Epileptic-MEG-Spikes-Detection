...
%  
%% ###########################################################################
function [Xsp,y_SE, y_CH,Time]=Extract_This_dataset_spikes_in_xlsx(Patient_Folder,Spikes_table)
global MEGChannels
...
for file_k=1:size(List_mat_files,1)
    ...
    %% Get the recordes saved  in MEG.F
        ...
        Data_SE=MEG.F;M=size(Data_SE,2);
        eval(strcat('Time.S',num2str(SE_Num),'=get_Time_from_Record(MEG.Comment,M);'));
%         Spikes_table.Time=Time;
