...
%  
%% ###########################################################################
function [DataX,CHs,ID_patient,ID_SE,L_max]=Extract_dataset_basedon_spikes(MEG_patient,L_max, Frame_Step,Time, X, y_SE, Spikes_table,fs)
...
for Session=List_of_Sessions'
    ...
    for Spik2study=1:size(Spike_stops,1)
       ...
       eval(strcat('t=Time.S',num2str(Session),'(1:M);'));
